...
export default function MyRequestCard({ myrequestLeave }) {
    const startDate = myrequestLeave?.startDate ? new Date(myrequestLeave.startDate) : null;
    const endDate = myrequestLeave?.endDate ? new Date(myrequestLeave.endDate) : null;
            ...
            <div className="flex h-full items-center p-2 justify-start gap-6 w-full">
                ...
                <div>
                    <div className="flex flex-col gap-2">
                        <h1 className="text-green-700 font-semibold">StartDate: {formatDate(startDate)}</h1>
                        <h1 className="text-green-700 font-semibold">EndDate: {formatDate(endDate)}</h1>
